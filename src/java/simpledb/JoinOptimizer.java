...
public class JoinOptimizer {
    ...
    public Vector<LogicalJoinNode> orderJoins(
    	...
    	for(Set<LogicalJoinNode> s : j) {
    		CostCard costcard = computeCostAndCardOfSubplan(stats, filterSelectivities, s.iterator().next(), s, Double.MAX_VALUE,planCache);
    		planCache.addPlan(s, costcard.cost, costcard.card, costcard.plan);
    	}
    	for(int i = 2; i <= joins.size(); i++) {
    		Set<Set<LogicalJoinNode>> n = enumerateSubsets(joins, i);
    		for(Set<LogicalJoinNode> s : n){
    			Vector<LogicalJoinNode> bestJoinOrder = null;
    			Double bestCost = Double.MAX_VALUE;
    			Integer bestCard = Integer.MAX_VALUE;
    	    			
    			for(LogicalJoinNode joinNode : joins) {
    				    				
    				CostCard  costCard = computeCostAndCardOfSubplan(stats, filterSelectivities, joinNode, s, bestCost, planCache);
    				if(costCard != null) {
    					if(costCard.cost < bestCost) {
    						bestJoinOrder = costCard.plan;
    			...
    			planCache.addPlan(s, bestCost, bestCard, bestJoinOrder);
